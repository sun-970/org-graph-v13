...
void main() {
  ...
  pearl += vec3(1.0, 0.97, 0.9) * (spec * 0.28 + spec2 * 0.32 + ndSpin * 0.08);
  pearl += vec3(0.92, 0.82, 0.52) * fresnel * 0.55;

  vec3 up = abs(V.y) < 0.96 ? vec3(0.0, 1.0, 0.0) : vec3(1.0, 0.0, 0.0);
  vec3 T = normalize(cross(up, V));
  vec3 B = cross(V, T);
  vec2 uv = vec2(dot(n, T), dot(n, B)) / (0.88 + 0.12 * facing);
  float r = length(uv);
  vec2 texUV = clamp(uv * 0.5 + 0.5, 0.0, 1.0);
  float mask = smoothstep(1.12, 0.9, r) * smoothstep(0.0, 0.12, facing);
  ...
  col += vec3(0.9, 0.8, 0.45) * selected * 0.12;
...
async function buildPeople() {
  ...
  PEOPLE.forEach((person, i) => {
    ...
    const color = new THREE.Color(0xcbb889);
